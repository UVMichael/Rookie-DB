...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                //get all locks in this transaction
                List<Lock> allLocks = lockManager.getLocks(this);
                //reverse the order of the locks so the children lock get released before the parent
                Collections.reverse(allLocks);
                //iterate over all locks.
                for(Lock currLock : allLocks){
                    //get the current context of the lock
                    LockContext currLockContext =  LockContext.fromResourceName(lockManager, currLock.name);
                    //release the current transactions lock on the curr lock resource.
                    currLockContext.release(this);
                }
                return;
            } catch (Exception e) {
