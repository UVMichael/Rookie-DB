    ...
    private class BNLJIterator implements Iterator<Record>{
        ...
        private void fetchNextLeftBlock() {
            ...
            if(!leftSourceIterator.hasNext()){
                // if no more records in left do nothing
                return;
            }
            //set the left block iterator,
            leftBlockIterator = getBlockIterator(leftSourceIterator,
                    getLeftSource().getSchema(), numBuffers - 2 );

            //set left record to first record, and mark it.
        ...
        private void fetchNextRightPage() {
            ...
            if(!rightSourceIterator.hasNext()){
                //if no more records do nothing
                return;
            }
            //fetch the next page of records from the right source and set that to be the rightpage iterator.
            rightPageIterator = getBlockIterator(rightSourceIterator, getRightSource().getSchema(), 1);

            //mark next ex. mark start
        ...
        private Record fetchNextRecord() {
            // TODO(proj3_part1): implement
            //early termination
            if (leftRecord == null || rightSourceIterator == null ) {
                return null;
            }
            while (true) {
                //if we have a record to the right move it.
                if (rightPageIterator.hasNext()) {
                    Record currRight = rightPageIterator.next();
                    //check current records
                    if(compare(leftRecord,currRight) == 0) {
                        //if match return concatinated and advance right record
                        return leftRecord.concat(currRight);
                    }
                }

                //if we ran out of records in the right page but have records in left buffer
                else if (leftBlockIterator.hasNext()) {
                //advance the left record pointer
                leftRecord = leftBlockIterator.next();
                //reset the right page iterator.
                rightPageIterator.reset();
                }

                //if we reached end of left and right but has more right pages
                else if (rightSourceIterator.hasNext()) {
                    //get next right page
                    fetchNextRightPage();
                    //reset left buffer and update teh left record
                    leftBlockIterator.reset();
                    leftRecord = leftBlockIterator.next();
                }

                //if we reached end of left buffer and right pages
