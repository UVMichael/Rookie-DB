...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        //get the estimated IO cost for preforming a sequential scan for the given table
        int minIO = minOp.estimateIOCost();

        //determine cost of an index scan
        int bestIndex = -1;
        //itteratre over all possible indexs
        for(int currentIndex: getEligibleIndexColumns(table)){
            //get the predicate of the current index
            SelectPredicate currPredicate = selectPredicates.get(currentIndex);
            //create a new plan and check its IO cost compared to others
            QueryOperator currPlan = new IndexScanOperator(this.transaction, table,
                    currPredicate.column, currPredicate.operator, currPredicate.value);
            //calc current IO cost
            int currCost = currPlan.estimateIOCost();
            //check if this io is less than
            if(currCost < minIO){
                //update fields
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        //For each set of tables in prevMap
        for (Set<String> curTable : prevMap.keySet()) {
            //For each join predicate listed in this.joinPredicates
            for (JoinPredicate currPred : this.joinPredicates) {
                //table generated by result of join.
                QueryOperator joinedTable;
                //table which will be fetched by pass1Map
                QueryOperator newTable;

                //set with the current table
                Set<String> withTable = new HashSet<>(curTable);
                //set without the current table
                Set<String> withoutTable = new HashSet<>();
                ...
                if (curTable.contains(leftTable) && !curTable.contains(rightTable)) {
                    //Case 1: The set contains left table but not right, use pass1Map
                    //to fetch an operator to access the rightTable
                    withoutTable.add(rightTable);
                    withTable.add(rightTable);
                    //set new table
                    newTable = pass1Map.get(withoutTable);
                    //use minCostJoinType to calculate the cheapest join with the new table
                    joinedTable = minCostJoinType(prevMap.get(curTable), newTable, leftColumn, rightColumn);


                } else if (!curTable.contains(leftTable) && curTable.contains(rightTable)) {
                    // Case 2: The set contains right table but not left, use pass1Map to fetch an operator
                    // to access the leftTable.
                    withoutTable.add(leftTable);
                    withTable.add(leftTable);
                    //set new table
                    newTable = pass1Map.get(withoutTable);
                    //use minCostJoinType to calculate the cheapest join with the new table
                    joinedTable = minCostJoinType(prevMap.get(curTable), newTable, rightColumn, leftColumn);
                ...
                //if key in the results but the tableJoin is better add this key.
                if(result.containsKey(withTable) && result.get(withTable).estimateIOCost() > joinedTable.estimateIOCost()){
                    result.put(withTable, joinedTable);
                }
                //insert key if not in the results
                if(!result.containsKey(withTable)){
                    result.put(withTable, joinedTable);
                }
            }
        }
    ...
    public Iterator<Record> execute() {
        ...
        //TODO(proj3_part2): Replace this!

        //pass 1
        //mapping of table to minimum cost single acces
        Map<Set<String>, QueryOperator> costMap = new HashMap<>();
        //iterate iver all the table names.
        for(String table : this.tableNames) {
            //create a new mapping for each table, used by const map
            Set<String> tableMap = new HashSet<>();
            //add table to table name map
            tableMap.add(table);
            //add table map to the costMap
            costMap.put(tableMap,minCostSingleAccess(table));
        }

        // pass i > 1
        Map<Set<String>, QueryOperator> minTable = costMap;
        //iterate over all the tables
        for(int i = 1; i < this.tableNames.size(); i++) {
            //repeatedly use results fromt eh previous pass to find the lowest cost join with each single table. repeat
            minTable = minCostJoins(minTable, costMap);
        }
        assert(minTable.size()== 1);
        //get lowest cost operator from teh last pass
        finalOperator=minCostOperator(minTable);
        //add group by, project, sort and limit operators
        addGroupBy();
        addProject();
        addSort();
        addLimit();
        //return iterator over final elem.
