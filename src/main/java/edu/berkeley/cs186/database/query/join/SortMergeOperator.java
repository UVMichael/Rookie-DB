    ...
    private class SortMergeIterator implements Iterator<Record> {
        ...
        */
        ...
        private Record fetchNextRecord() {
            ...
            //base case no more records on left
            if(leftRecord == null || rightRecord == null){
                return null;
            }
            //check if the we are in marked case, ie we have gone down path
            if(!marked){
                //ensure that left and right dont switch
                while((compare(leftRecord, rightRecord) > 0 && rightIterator.hasNext()) ||
                        (compare(leftRecord, rightRecord) < 0 && leftIterator.hasNext())) {
                    //go over the records which are leftRecord's join value is greater than rightRecord
                    while (compare(leftRecord, rightRecord) > 0 && rightIterator.hasNext()) {
                        rightRecord = rightIterator.next();
                    }
                    // go over the records which are right records join value is greater than left
                    while (compare(leftRecord, rightRecord) < 0 && leftIterator.hasNext()) {
                        leftRecord = leftIterator.next();
                    }
                }
                //at a new eq value mark prev and continue
            ...
            if(compare(leftRecord,rightRecord) == 0){
                ...
                if(rightIterator.hasNext()){
                    ...
                    //reset right on advancement of left.
                    rightIterator.reset();
                    rightRecord = rightIterator.next();
                }
                return newRecord;
