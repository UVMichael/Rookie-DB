...
class LeafNode extends BPlusNode {
    ...
    public LeafNode getLeftmostLeaf() {
        // TODO(proj2): implement
        //a leaf node may have no children so it must be its own leftmost leaf
        return this;
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        //check valid key ie no duplicate keys
        if(this.keys.contains(key)) {
            throw new BPlusTreeException("duplicate key inserted");
        }

        int entryIndex= InnerNode.numLessThanEqual(key, keys);
        //add record and key to the LeafNode
        this.keys.add(entryIndex, key);
        this.rids.add(entryIndex, rid);
        int order = this.metadata.getOrder();
        //check if overflow occurs
        if (this.keys.size() <= order * 2) {
            //sync to update the node
            sync();
            //return empty since no additional node was added since no overflow
            return Optional.empty();
        }

        // If overflow create a new node.
        //create new keys and new records
        List<DataBox> newKeys = new ArrayList<>();
        List<RecordId> newRids = new ArrayList<>();

        //migrate elements over the order from current ListNode to new Node
        ...
        LeafNode newNode = new LeafNode(this.metadata, this.bufferManager, newKeys,
                newRids, this.rightSibling, this.treeContext);

        //get new physical page number associated with the newly created node.
        long newPage = newNode.getPage().getPageNum();
        //set the rightsibling of the current node to be the page of the  newly created node
        this.rightSibling = Optional.of(newPage);
        //sync to update the nodes.
        sync();
        //return the pagenumber of the new node as well as the leftmost key for higher propogation.
        return Optional.of(new Pair<>(newNode.keys.get(0), newPage));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        //check valid fillfactor and order.
        if (fillFactor * 2 * order <= 0) {
            throw new BPlusTreeException("invalid fillfactor for order.");
        }

        //calculate  number of keys which can be inserted at this leaf node.
        int totalKeys = (int) Math.ceil(2*order*fillFactor);

        //fill up leaves to size total keys or until data is empty
        for (int i = keys.size(); i < totalKeys && data.hasNext(); i++) {
            //get next data elem
            Pair<DataBox, RecordId> currData = data.next();
            //insert data elem into the current node
            this.keys.add(currData.getFirst());
            this.rids.add(currData.getSecond());
        }
        //if we have imputed all the data
        if (!data.hasNext()) {
            sync();
            return Optional.empty();
        }
        //if there is data left to be imputed and we need to bubble up for a new node.
        //create new variables for new node
        List<DataBox> newKeys = new ArrayList<>();
        List<RecordId> newRids = new ArrayList<>();

        //fill nodes with initial data.
        Pair<DataBox, RecordId> pair = data.next();
        newKeys.add(0, pair.getFirst());
        newRids.add(0, pair.getSecond());

        //create new leaf node.
        LeafNode newLeaf = new LeafNode(this.metadata, this.bufferManager, newKeys, newRids,
                Optional.empty(), this.treeContext);

        //get associated page number of new node
        Long pageNum = newLeaf.getPage().getPageNum();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        //from inner node::fromBytes
        Page page = bufferManager.fetchPage(treeContext, pageNum);
        Buffer buf = page.getBuffer();

        byte nodeType = buf.get();

        //check from part a that nodeType is a leafnode
        ...
        //set right sibling
        Long siblingLogic = buf.getLong();
        if(siblingLogic == -1) {
            rightSibling = Optional.empty();
        } else {
            rightSibling = Optional.of(siblingLogic);
        }

        //get number of enteries from part C
