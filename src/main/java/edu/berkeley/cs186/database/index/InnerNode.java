...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        //get the associated key index for the leaf nodes available children
        int keyIndex = numLessThanEqual(key, this.getKeys());

        //read associated node into memory.
        BPlusNode currNode = getChild(keyIndex);

        //traverse tree until hit a leafNode
        while (!(currNode instanceof LeafNode)) {
            currNode = currNode.get(key);
        }
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        // TODO(proj2): implement
        //get left most child of current branch
        BPlusNode currChild  = this.getChild(0);
        //while not a leafNode
        while (!(currChild instanceof LeafNode)){
            //recurse down the children
            currChild = currChild.getLeftmostLeaf();
        }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        //find index at where to put the new key.
        int childKeyIndex = numLessThanEqual(key, this.keys);
        //get child at specified node
        BPlusNode childNode = getChild(childKeyIndex);
        //run recursive alg
        Optional<Pair<DataBox, Long>> logic = childNode.put(key, rid);

        //logic if we need to reorder this inner node.
        if (!logic.isPresent()) {
            //FIXME?? unneeded synch
            sync();
            return Optional.empty();
        }

        //retrieve data from the logic
        DataBox newLeftKey = logic.get().getFirst();
        long pageNumber = logic.get().getSecond();

        //insert result from the previous function call
        this.keys.add(childKeyIndex, newLeftKey);

        //add the respective page number
        this.children.add(childKeyIndex + 1, pageNumber);
        ...
        //case overflow
        //create new variables for new InnerNode
        List<DataBox> newKeys = this.keys.subList(order+1, 2*order+1);
        List<Long> newChildren = this.children.subList(order+1, 2*order+2);

        //get the new keyas and children of the current node
        List<DataBox> oldKeys = this.keys.subList(0, order);
        List<Long> oldChildren = this.children.subList(0, order+1);

        //get the new leftmost key for the new node
        DataBox newKey = this.keys.get(order);

        //create a new inner node
        InnerNode newInnerNode = new InnerNode(this.metadata,  this.bufferManager,
                newKeys, newChildren, this.treeContext);

        //update the current node
        this.keys = oldKeys;
        this.children = oldChildren;

        //generate the page of teh new node
        long newPage = newInnerNode.getPage().getPageNum();

        sync();
        return Optional.of(new Pair<>(newKey, newPage));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        //check we still have more data and the node is not surpased the 2d.
        //fill factor only matters on leaves, fill up all data
        while (data.hasNext() && this.keys.size() <= 2*order) {
            //get right most child
            BPlusNode lastChild = getChild(this.children.size() - 1);
            Optional<Pair<DataBox, Long>> result = lastChild.bulkLoad(data, fillFactor);
            //check if recursive bulkLoad bubbled up a node
            if (result.isPresent()) {
                //add bubbled up data to the current node
                this.keys.add(this.keys.size(), result.get().getFirst());
                this.children.add(this.children.size(), result.get().getSecond());
            }
        }

        //check if addition caused an overflow
        if (keys.size() <= 2 * order) {
            //valid bulk loading no need to bubble up anymore.
            sync();
            return Optional.empty();
            ...
            InnerNode newNode = new InnerNode(metadata, this.bufferManager, newKeys, newChildren, this.treeContext);
            ...
            return Optional.of(new Pair<>(newKey, newNode.getPage().getPageNum()));
        }
    }
