...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        //get leaf node with the possible associated key.
    ...
    public Iterator<RecordId> scanAll() {
        ...
        LeafNode leftLeaf = root.getLeftmostLeaf();
        //return a new iterator
        return new BPlusTreeIterator(leftLeaf, leftLeaf.scanAll());
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        //get items greater than or equal to that node
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> logic = root.put(key, rid);
        ...
        if (logic.isPresent()) {
            ...
            newKeys.add(logic.get().getFirst());
            newChildren.add(root.getPage().getPageNum());
            newChildren.add(logic.get().getSecond());

            BPlusNode newRoot = new InnerNode(this.metadata, this.bufferManager, newKeys, newChildren, this.lockContext);
            updateRoot(newRoot);
        }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        //check if tree is empty if not throw.
        LeafNode leftNode = this.root.getLeftmostLeaf();
        //make sure the leftmost node is the root with no other leaves.
        if (leftNode != this.root) {
            throw new BPlusTreeException("bulk load on tree with left most node not being root");
        }
        //make sure no other leaves
        if (leftNode.scanAll().hasNext()){
            throw new BPlusTreeException("bulk load on non-empty tree");
        }

        //loop over data
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> result = this.root.bulkLoad(data, fillFactor);
            //if we need to bubble up and make a new root node.
            if (result.isPresent()) {
                //create new fields for a new inner node
                List<DataBox> newKeys = new ArrayList<>();
                List<Long> newChildren = new ArrayList<>();

                //load the bubbled elem to the keys
                newKeys.add(result.get().getFirst());

                //add the new children plus previous root.
                newChildren.add(root.getPage().getPageNum());
                newChildren.add(result.get().getSecond());

                InnerNode inner = new InnerNode(this.metadata, this.bufferManager, newKeys, newChildren, this.lockContext);
                //switch root
                updateRoot(inner);
            }
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        //tracker for the current leaf
        private LeafNode currLeaf;
        //iterator for the leafs
        private Iterator<RecordId> iter;

        /** constructor for the BplusTree Iterator given a leaf. **/
        public BPlusTreeIterator(LeafNode leaf, Iterator<RecordId> iter){
            //assume good faith that leaf or iter are not null. FIXME

            this.currLeaf = leaf;
            //scan current leaf for nodes.
            this.iter = iter;

            //check if we are starting on an empty leaf
        ...
        /** advances the BPlusIterator to the next node
         * should only be called when the current leaf is empty
         **/
        private void toNextLeaf() {
            Optional<LeafNode> nextNode = currLeaf.getRightSibling();
            if(nextNode.isPresent()){
                // set the new node.
                this.currLeaf = nextNode.get();
                //load values into the iter
                this.iter = this.currLeaf.scanAll();

                //check if the new leafNode is empty
                if(!this.iter.hasNext()){
                    toNextLeaf();
                }
            } else {
                //case that there are no more siblings null out values
                this.currLeaf = null;
                this.iter = null;
            }
        }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            //should only be null if set by toNextLeaf
            return iter != null;
        }
        ...
        public RecordId next() {
            ...
            //throw if no next element availible
            if (!this.hasNext()) {
                throw new NoSuchElementException();
            }
            RecordId result = iter.next();
            if (!iter.hasNext()){
                toNextLeaf();
            }
            return result;
